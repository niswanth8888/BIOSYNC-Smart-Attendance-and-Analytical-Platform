...
# ==================================================
# ⭐ SAFE CONSOLE + FORCE UI WORKING DIRECTORY
# ==================================================
...
CONFIG_FILE = os.path.join(BASE_DIR, "config", "sdk_config.json")
SDK_LAST_STATUS_FILE = os.path.join(BASE_DIR, "config", "sdk_last_status.json")
...
if os.path.exists(CONFIG_FILE):
    ...
    TARGET_DEVICES = cfg.get("TARGET_DEVICES")
    AUTO_CLEAR_LOGS = cfg.get("AUTO_CLEAR_LOGS", False)

    fd = cfg.get("FETCH_DATE")

    # ⭐ SAFE FETCH DATE
    if fd in ["", "null", "None", None]:
        FETCH_DATE = None
    else:
        FETCH_DATE = fd
...
def make_json_safe_list(values):
    return [str(v) for v in values]

def save_sdk_last_status():
    try:
        os.makedirs(os.path.dirname(SDK_LAST_STATUS_FILE), exist_ok=True)

        status_payload = {
            "successful_devices": make_json_safe_list(successful_devices),
            "offline_devices": make_json_safe_list(offline_devices),
            "no_log_devices": make_json_safe_list(no_log_devices),
            "error_devices": make_json_safe_list(error_devices),
            "last_run": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }

        with open(SDK_LAST_STATUS_FILE, "w", encoding="utf-8") as f:
            json.dump(status_payload, f, indent=4, ensure_ascii=False)

        print(f"📝 Latest SDK status saved to {SDK_LAST_STATUS_FILE}")

    except Exception as e:
        print(f"⚠ Failed to save SDK last status JSON: {e}")

# ==================================================
# EXCEL COMPRESSION (UNCHANGED)
# ==================================================
...
def pull_device(device):
    ...
    try:
        ...
        if not filtered:
            ...
            log_device(name, ip, f"UPDATED {updated}")
